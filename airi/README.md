...
## Echo Memory (EchoCast Integration)

> **로컬 포크 전용** — 이 패키지는 `packages/echo-memory`에 위치한 커스텀 추가 패키지입니다.
...
[Fast-path Filter]  정규식 기반 즉시 drop (ㅋㅋ / 이모지 / 스팸)
  ↓ pass
[sLLM Bouncer]      llama.cpp HTTP → ignore / pass / rag / interrupt
  ↓ pass
...
│   ├── logger.ts                 LLM API 콜/응답 정밀 로거 (BOUNCER / SUMMARIZER / MAIN)
│   ├── memory/
...
- [`analysis/airi_llm_triggers.md`](./analysis/airi_llm_triggers.md) — LLM 인퍼런스 트리거 & 컨텍스트 주입 구조 분석
- [`analysis/echocast_migration_plan.md`](./analysis/echocast_migration_plan.md) — EchoCast 기능 이식 로드맵 (P0~P8)
- [`analysis/input_flow_overview.md`](./analysis/input_flow_overview.md) — 채팅/오디오 입력 플로우 개요
